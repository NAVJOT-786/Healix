...
def call_openai_compatible_text(
    provider_name: str, api_key: str, base_url: str,
    model: str, prompt: str, timeout: int = 45,
) -> str:
    resp = requests.post(
        f"{base_url}/chat/completions",
        headers={
...
def call_ollama_text(prompt: str, timeout: int | None = None) -> str:
    resp = requests.post(
        f"{OLLAMA_URL}/api/generate",
        json={
            "model": OLLAMA_MODEL,
...
def build_provider_registry() -> dict[str, ProviderDict]:
    ...
    registry["ollama"] = {
        ...
        "chat": lambda p: call_ollama_text(p, CHAT_TIMEOUT_SEC),
        "model": OLLAMA_MODEL,
    }
    registry["groq"] = {
        ...
        "chat": lambda p: call_openai_compatible_text(
            "groq", GROQ_API_KEY, GROQ_BASE_URL, GROQ_MODEL, p, CHAT_TIMEOUT_SEC),
        "model": GROQ_MODEL,
    }
    registry["cerebras"] = {
    ...
    registry["openrouter"] = {
        ...
        "chat": lambda p: call_openai_compatible_text(
            "openrouter", OPENROUTER_API_KEY, OPENROUTER_BASE_URL,
            OPENROUTER_MODEL, p, CHAT_TIMEOUT_SEC),
...
def chat_with_providers(
    ...
    """
    Plain-text chat variant: iterate the provider chain and return free text.
    Returns (reply_text, provider_name) or (None, "unknown").
    """
    for provider_name in chain or DIAGNOSIS_PROVIDER_CHAIN:
        provider = registry.get(provider_name)
        if not provider or not provider["enabled"]:
            continue

        log.info("Chat: trying %s (%s)...", provider_name, provider["model"])
        start = time.time()
        ...
        try:
            if provider_name == "gemini":
                ...
                chat_call = provider.get("chat")
                assert chat_call is not None
                raw = chat_call(prompt)
            ...
            elapsed = time.time() - start
            log.info("✓ chat %s responded in %.1fs", provider_name, elapsed)
            return raw, provider_name

        except requests.exceptions.RequestException as e:
            log.warning("⚠ chat %s unreachable (%s) — trying next...", provider_name, e)
        except Exception as e:
            log.warning("⚠ chat %s error (%s) — trying next...", provider_name, e)
